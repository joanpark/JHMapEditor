...
const Pattern = {
    ...
    extrapolateEndPoints() {
        ...
        const reducedData   = this.data.reduce((a, b) => a.concat(b), []).sort((a, b) =>  a - b);
        ...
        const topLeftCoords     = { row: Math.min(row1, row2), col: Math.min(col1, col2) };
        // const bottomRightCoords = { row: Math.max(row1, row2), col: Math.max(col1, col2) };
    ...
    render() {
        ...
        for (var row = 0; row < this.TILES_HIGH; row++) {
            for (var col = 0; col < this.TILES_WIDE; col++) {
                ...
                var cell = (row * this.TILES_WIDE) + col;
                var tile = this.data[cell];
